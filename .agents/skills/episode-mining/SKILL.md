...
찾은 내용은 아래 **3층으로 분리**해서 정리한다.

1. **증거층**: 문서/코드/커밋에서 직접 확인한 사실
2. **해석층**: 그 사실이 왜 차별점인지, 사용자/운영자/업무에 왜 중요한지
3. **서술층**: 자소서에 넣을 수 있는 사람말 문장

### 핵심 규칙

- 메서드명, 클래스명, 컬럼명, 내부 상태값 같은 **구현 디테일은 기본적으로 증거층에만 둔다**
- 자소서 본문에는 구현명을 그대로 쓰지 말고 **업무 의미로 번역**한다
- 기술 포인트는 "무엇을 썼다"보다 **왜 그게 중요한 설계였는가**로 바꾼다
- 코드 2차 탐색은 **차별점의 근거 확보용**이지, 자소서 문장을 기술 면접 답변처럼 만드는 단계가 아니다

### 작성물별 사용 범위

- **에피소드 정리 문서**: 증거층 + 해석층 + 서술층을 모두 남긴다
- **자소서 초안 작성 단계**: 해석층을 보고 서술층을 문항 의도에 맞게 고른다
- **최종 자소서 본문**: 기본적으로 서술층만 쓴다
- **면접 준비 메모**: 증거층을 다시 참고한다

예시:
- 증거층: `AFTER_COMMIT`, `uk_jobs_idempotency`, `PENDING→RUNNING`
- 해석층: 트랜잭션 경계와 중복 실행 위험까지 함께 설계함
- 서술층: 비동기 처리 구조를 만들 때 응답 속도뿐 아니라 실행 안정성까지 함께 고려했다
...
단, 이 단계는 아직 **증거 수집 단계**이므로 구현 디테일을 그대로 보관해도 된다.

상세 가이드: [references/project-scan-guide.md](references/project-scan-guide.md) 의 "기술 심화 추출 레이어" 섹션

### A-2.6. 코드 2차 탐색 → 차별점 번역

문서·README·설계 자료로 1차 스캔한 뒤, **코드 2차 탐색**으로 실제 차별 포인트를 확인한다.

이 단계의 목표는 "더 자세한 기술 설명"이 아니라, **3층 구조로 번역 가능한 차별 포인트인지** 판별하는 것이다.

이 단계에서는 반드시 아래 질문을 거친다.
- 이 구현 디테일이 없으면 사용자나 운영자 입장에서 어떤 문제가 생기는가?
- 이 설계는 흔한 기술 나열이 아니라 어떤 판단을 보여주는가?
- 이 표현을 자소서에 넣었을 때 너무 지엽적이지 않은가?
- 메서드명/컬럼명을 빼도 의미가 살아 있는가?
...
최종 산출물은 앞서 정의한 **3층 구조**를 유지한다.

자소서 서술 후보 작성 규칙:
- 구현명보다 설계 판단을 쓸 것
- 로컬 변수·테이블·메서드 수준의 표현은 제거할 것
- 사용자/운영자/조직/업무 기준으로 의미를 바꿔 쓸 것
- 한 문장 안에 기술 포인트를 2개 이하만 담을 것
...
대화 기반에서도 결과물은 동일하게 **증거층·해석층·서술층**으로 정리한다.

- **증거층**: 사용자가 직접 말한 사실, 기억, 상황, 수치, 표현
- **해석층**: 그 경험이 보여주는 강점, 판단, 사용자 가치, 업무 의미
- **서술층**: 자소서에 넣을 수 있는 문장

대화 기반에서는 코드나 문서가 없을 수 있으므로, 증거층은 아래처럼 다룬다.
- 사용자가 직접 진술한 사실은 **사용자 진술 기반 증거**로 기록한다
- 기억이 모호하거나 수치가 불확실하면 **확인 필요**로 표시한다
- 해석층으로 넘어갈 때는 사용자의 원래 표현을 과장하지 않는다

예시:
- 사용자 진술: "API가 자꾸 느려서 비동기로 바꿨어요"
- 증거층: 동기 처리 병목 경험, 비동기 전환 수행
- 해석층: 처리 지연을 기능 문제가 아니라 구조 문제로 보고 개선한 경험
- 서술층: 처리 지연의 원인을 구조적으로 보고, 요청과 실행을 분리하는 방식으로 개선했습니다.
...
인수분해 후에는 반드시 아래 순서로 한 번 더 정리한다:
- **증거층**: 사용자가 실제로 말한 사실만 남겼는가
- **해석층**: 이 경험의 의미를 과장 없이 설명했는가
- **서술층**: 자소서 본문에 바로 넣어도 지엽적이지 않은가
...
- 발굴한 에피소드 전체 목록, 14항목 인수분해, 기술 디테일, 3층 구조, 문항 매핑을 채운다.
